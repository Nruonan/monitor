...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (uri.startsWith("/monitor")){
            if (!uri.endsWith("/register")){
                ClientDO clientDO = service.findClientByToken(authorization);
                if (clientDO == null){
                    ...
                    request.setAttribute(Const.ATTR_CLIENT, clientDO);
                }
            }
        }else{
    ...
    private boolean accessShell(int userId, String userRole, int clientId) {
        if(Const.ROLE_ADMIN.equals(userRole.substring(5))) {
            ...
            AccountDO account = accountService.getById(userId);
