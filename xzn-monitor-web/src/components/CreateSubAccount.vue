...
import {osNameToIcon} from "@/tool";
import {post} from "@/net";
import {ElMessage} from "element-plus";
import {onCheck} from "@/method/client";
  ...
  <div style="padding: 15px 20px;height: 100%">
    <div style="display: flex; flex-direction: column;height: 100%">
      ...
      <el-scrollbar style="flex: 1">
        <div class="client-card" v-for="item in props.clients" :key="item.id">
          <el-checkbox @change="state => onCheck(checkedClients,state, item.id)"/>
          <div style="margin-left: 20px">
            <div style="font-size: 14px;font-weight: bold">
              <span :class="`flag-icon flag-icon-${item.location}`"/>
